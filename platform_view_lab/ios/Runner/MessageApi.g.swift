...
protocol MessageApi {
  func getRoundedNumberFromNative(value: Double, completion: @escaping (Result<String, Error>) -> Void)
}
...
class MessageApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: MessageApi?, messageChannelSuffix: String = "") {
    ...
    let getRoundedNumberFromNativeChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.pigeon_message_api.MessageApi.getRoundedNumberFromNative\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      getRoundedNumberFromNativeChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let valueArg = args[0] as! Double
        api.getRoundedNumberFromNative(value: valueArg) { result in
          switch result {
      ...
      getRoundedNumberFromNativeChannel.setMessageHandler(nil)
    }
  }
}
/// Generated protocol from Pigeon that represents Flutter messages that can be called from Swift.
protocol ReverseMessageApiProtocol {
  func getTemperatureInCityFromFlutter(city cityArg: String, completion: @escaping (Result<Int64, PigeonError>) -> Void)
}
class ReverseMessageApi: ReverseMessageApiProtocol {
  ...
  func getTemperatureInCityFromFlutter(city cityArg: String, completion: @escaping (Result<Int64, PigeonError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.pigeon_message_api.ReverseMessageApi.getTemperatureInCityFromFlutter\(messageChannelSuffix)"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
    channel.sendMessage([cityArg] as [Any?]) { response in
